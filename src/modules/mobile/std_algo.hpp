...
std::pair<ForwardIt, Size>
find_if_n2(ForwardIt first, Size count, Predicate p)
{
	Size i = 0;
	while (i < count and not p(*first))
	{
		++i;
		++first;
	}
...
template<class ForwardIt, class Size, class Predicate>
bool
all_of_n(ForwardIt first, Size count, Predicate p)
{
	// return find_if_n2(first, count, not1(p));
	// return find_if_not_n2(first, count, p);
	Size i = 0;
	while (i < count and p(*first))
	{
		++i;
		++first;
	}
